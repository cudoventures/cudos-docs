...
## How to Deploy a Smart Contract on fetch using `fetchd` - Fully Explained!
>**NOTE:** As always, if you see angle brackets surrounding text, eg: `<your-wallet-name>`, you need to fill in your own variable name.

1. **Install the fetch node**

    The fetch node daemon, `fetchd`, can be installed from here: [`fetchd`](https://github.com/fetchai/fetchd/).
...
3. **Set environment variables**
    ...
    :::info
    For up to date flags for fetch (and other Cosmos blockchains), you can refer to the [Cosmos Chain Registry](https://github.com/cosmos/chain-registry). *(It's often good practice when building frontends to pull the chain info automatically from the `chain-registry` repo so that your configuration stays up-to-date).*
    :::


4. **Create a wallet address and add it to the CLI instance.**

    Create a fetch wallet address, then add your mnemonic to your `fetchd` instance to make transactions on the chain with your private key *(remove the `_TN` from the end of any flags if you are working with the mainnet)*:
    ```console
    fetchd keys add <your-wallet-name> --keyring-backend $KEYRING_TN --recover
    ```
    ...
    ```console
    OWNER_TN=$( fetchd keys show -a <your-wallet-name> --keyring-backend "$KEYRING_TN" | tee /dev/tty | tail -1 | tr -d '\r' )
    ```

    >:warning: As always, keep your mnemonic phrase safe and secret.

    > Your wallet will also need funds to pay for gas fees, you can add testnet funds via the faucet.


5. **Deploy your compiled `.wasm` contract from before to the fetch blockchain.**

     To do this we use the `fetchd` CLI to run the `tx wasm store` command which uploads the contract file to the chain. We set the output of that command to the `$RESULT` environment variable:
    ```console
    STORE_RESULT=$(fetchd tx wasm store ./<path-to>/artifacts/<name-of-wasm-file.wasm> --from $OWNER_TN `echo $TX_FLAGS_TN`)
    ```
...
7. **Instantiate your contract**
    ...
    ```console
    INST_RESPONSE=$(fetchd tx wasm instantiate $CONTRACT_INDEX "$INST" --from $OWNER_TN --label $CONTRACT_LABEL `echo $TX_FLAGS_TN` $ADMIN)
    ```
...
8. **Get the contract address.**
    ...
    ```
    **You have successfully deployed and instantiated a smart contract on fetch! Congratulations!**
    >Having deployed and retrieved the contract address, you can now interact with the contract on the chain, you can do this with a frontend to build your project into a full decentralised application (dApp). Take a look at `create-cosmos-app` to get going, it's a simple way of scaffolding a React frontend within the Cosmos ecosystem - fetch included. [This YouTube video](https://www.youtube.com/live/hPec5D_lI1A?feature=share&t=1880) helps use `create-cosmos-app` for fetch.

9. **BONUS: Execute functions on the contract from the CLI**

    While most end users will interact with the smart contract via a dApp frontend, you can also interact and execute functions on the contact with the `fetchd` CLI. Here's an example using the CW20 standard token contract again:
    ...
    ```console
    OLLIE="fetch1d7jw3ply86e4kcudmlv78ask32kdp7r2usf8xn"
    ```
    ...
    ```console
    fetchd tx wasm execute $CONTRACT_ADDRESS "$TRANSFER_TO_OLLIE" --from $OWNER_TN `echo $TX_FLAGS_TN`
    ```
...
10. **BONUS 2: Query the contract state from the CLI**
    ...
    ```console
    fetchd query wasm contract-state smart $CONTRACT_ADDRESS "$BALANCE_OF_OLLIE" --node $RPC_NODE_TN
    ```
